...
func TestDialTimeout2SCTP(t *testing.T) {
...
func TestDialTimeoutMaxDurationSCTP(t *testing.T) {
	ln := newLocalListenerSCTP(t, "sctp")
	defer func() {
		if err := ln.Close(); err != nil {
			t.Error(err)
...
func TestAcceptTimeoutSCTP(t *testing.T) {
	ln := newLocalListenerSCTP(t, "sctp")
	defer ln.Close()
	...
	for i, tt := range acceptTimeoutTests {
		...
		if err := ln.(*SCTPListener).SetDeadline(time.Now().Add(tt.timeout)); err != nil {
			t.Fatalf("$%d: %v", i, err)
		}
...
func TestAcceptTimeoutMustReturnSCTP(t *testing.T) {
	ln := newLocalListenerSCTP(t, "sctp")
	defer ln.Close()

	if err := ln.(*SCTPListener).SetDeadline(noDeadline); err != nil {
		t.Error(err)
	}
	if err := ln.(*SCTPListener).SetDeadline(time.Now().Add(10 * time.Millisecond)); err != nil {
		t.Error(err)
	}
	c, err := ln.Accept()
	if err == nil {
...
func TestAcceptTimeoutMustNotReturnSCTP(t *testing.T) {
	ln := newLocalListenerSCTP(t, "sctp")
	defer ln.Close()
	...
	ch := make(chan error)
	go func() {
		if err := ln.(*SCTPListener).SetDeadline(time.Now().Add(-5 * time.Second)); err != nil {
			t.Error(err)
		}
		if err := ln.(*SCTPListener).SetDeadline(noDeadline); err != nil {
			t.Error(err)
...
func TestReadTimeoutSCTP(t *testing.T) {
...
func TestReadTimeoutMustNotReturnSCTP(t *testing.T) {
	ln := newLocalListenerSCTP(t, "sctp")
	defer ln.Close()
...
func TestReadMsgTimeoutSCTP(t *testing.T) {
	handler := func(ls *localServerSCTP, ln net.Listener) {
		c, err := ln.Accept()
		if err != nil {
			t.Error(err)
	...
	for i, tt := range readTimeoutTests {
		...
		for j, xerr := range tt.xerrs {
			for {
				n, _, _, err := c.(*SCTPConn).ReadMsg(b[:])
				if xerr != nil {
					if perr := parseReadError(err); perr != nil {
						t.Errorf("#%d/%d: %v", i, j, perr)
					}
...
func TestWriteTimeoutSCTP(t *testing.T) {
	ln := newLocalListenerSCTP(t, "sctp")
	defer ln.Close()
...
func TestWriteTimeoutMustNotReturnSCTP(t *testing.T) {
	ln := newLocalListenerSCTP(t, "sctp")
	defer ln.Close()
...
func TestWriteMsgTimeoutSCTP(t *testing.T) {
	ln := newLocalListenerSCTP(t, "sctp")
	defer ln.Close()
	...
	for i, tt := range writeTimeoutTests {
		...
		for j, xerr := range tt.xerrs {
			for {
				n, err := c.(*SCTPConn).WriteMsg([]byte("WRITE TIMEOUT TEST"), nil)
				if xerr != nil {
					if perr := parseWriteError(err); perr != nil {
						t.Errorf("#%d/%d: %v", i, j, perr)
					}
...
func TestReadTimeoutFluctuationSCTP(t *testing.T) {
	ln := newLocalListenerSCTP(t, "sctp")
	defer ln.Close()
...
func TestReadMsgTimeoutFluctuationSCTP(t *testing.T) {
	ln := newLocalListenerSCTP(t, "sctp")
	defer ln.Close()
...
func TestWriteTimeoutFluctuationSCTP(t *testing.T) {
	ln := newLocalListenerSCTP(t, "sctp")
	defer ln.Close()
...
func TestConcurrentSetDeadlineSCTP(t *testing.T) {
	ln := newLocalListenerSCTP(t, "sctp")
	defer ln.Close()
